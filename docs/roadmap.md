...
* stabilize CLI arguments, JSON output, and exit-code behavior
* define analyzer result schema and diagnostic rules
* prepare a stable integration contract before Java coupling
...
* add a minimal Java adapter for calling the Rust executable
* handle process execution, timeout, stderr, and JSON parsing
* keep the live camera pipeline unchanged
...
* introduce selectable calculation-engine architecture
* support Java and optional Rust calculation backends
* persist engine metadata in calculation runs
* keep Java calculation as the default baseline
...
* integrate Rust analysis into the recalculation workflow
* execute Rust calculations from persisted delta data
* persist Rust calculation results beside Java runs
* keep runtime failures isolated and non-fatal
